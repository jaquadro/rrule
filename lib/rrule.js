...
var dateutil = {
    ...
    getYearDay: function(date) {
        var dateNoTime = new dateutil.Date(
            date.getFullYear(), date.getMonth(), date.getDate());
        return Math.ceil(
            (dateNoTime - new dateutil.Date(date.getFullYear(), 0, 1)) / dateutil.ONE_DAY) + 1;
    },

    isLeapYear: function(year) {
        if (year instanceof dateutil.Date) {
    ...
    fromOrdinal: function(ordinal) {
        ...
        return new dateutil.Date(dateutil.ORDINAL_BASE.getTime()
                        - dateutil.tzOffset(dateutil.ORDINAL_BASE)
                        +  millisecsFromBase
                        + dateutil.tzOffset(new dateutil.Date(millisecsFromBase)));
    },
    ...
    monthRange: function(year, month) {
        var date = new dateutil.Date(year, month, 1);
    ...
    combine: function(date, time) {
        time = time || date;
        return (date instanceof Date || !time.getTimezone())
            ? new dateutil.Date(
                date.getFullYear(), date.getMonth(), date.getDate(),
            ...
            : new dateutil.Date(
                date.getFullYear(), date.getMonth(), date.getDate(),
                time.getHours(), time.getMinutes(), time.getSeconds(), time.getTimezone()
            );
    },

    clone: function(date) {
        var dolly = new dateutil.Date(date.getTime());
    ...
    timeToUntilString: function(time) {
        var date = new dateutil.Date(time);
        var comp, comps = [
            date.getUTCFullYear(),
            date.getUTCMonth() + 1,
            date.getUTCDate(),
...
var RRule = function(options, noCache) {
    ...
    if (!opts.dtstart) {
        opts.dtstart = new dateutil.Date();
        opts.dtstart.setMilliseconds(0);
    }
    else if (dateutil.Date !== Date) {
        opts.dtstart = new dateutil.Date(opts.dtstart, opts.timezone);
    }
        ...
        for (i = 0; i < opts.byhour.length; i++) {
            ...
            for (var j = 0; j < opts.byminute.length; j++) {
                ...
                for (var k = 0; k < opts.bysecond.length; k++) {
                    ...
                    // tzinfo=self._tzinfo))
                    this.timeset.push(new dateutil.Time(hour, minute, second, opts.timezone));
                }
            }
        }
...
RRule.prototype = {
    ...
    */
    ...
    _cacheAdd: function(what, value, args) {
        ...
        if (value) {
            value = (value instanceof dateutil.Date)
    ...
    _cacheGet: function(what, args) {
        ...
        return cached instanceof Array
            ...
            : (cached instanceof dateutil.Date
...
Iterinfo.prototype.rebuild = function(year, month) {
    ...
    if (year != this.lastyear) {
        ...
        var firstyday = new dateutil.Date(year, 0, 1);

        this.yearordinal = dateutil.toOrdinal(firstyday);
        this.yearweekday = dateutil.getWeekday(firstyday);

        var wday = dateutil.getWeekday(new dateutil.Date(year, 0, 1));
...
Iterinfo.prototype.wdayset = function(year, month, day) {
    ...
    var i = dateutil.toOrdinal(
        new dateutil.Date(year, month - 1, day)) - this.yearordinal;
    var start = i;
    for (var j = 0; j < 7; j++) {
        set[i] = i;
        ++i;
...
Iterinfo.prototype.htimeset = function(hour, minute, second) {
    ...
    for (var i = 0; i < rr.options.byminute.length; i++) {
        ...
        for (var j = 0; j < rr.options.bysecond.length; j++) {
            ...
            set.push(new dateutil.Time(hour, minute, second, rr.options.timezone));
        }
    }
    dateutil.sort(set);
    return set;
...
Iterinfo.prototype.stimeset = function(hour, minute, second) {
    return [new dateutil.Time(hour, minute, second, this.rrule.options.timezone)];
};
...
IterResult.prototype = {
    ...
    init: function(method, args) {
        ...
        if (method == 'between') {
            this.maxDate = args.inc
                ? args.before
                : new dateutil.Date(args.before.getTime() - 1);
            this.minDate = args.inc
                ? args.after
                : new dateutil.Date(args.after.getTime() + 1);
        } else if (method == 'before') {
            this.maxDate = args.inc ? args.dt : new dateutil.Date(args.dt.getTime() - 1);
        } else if (method == 'after') {
            this.minDate = args.inc ? args.dt : new dateutil.Date(args.dt.getTime() + 1);
        }
    },
     ...
     */
    add: function(date) {
        this._result.push( new Date(date.valueOf()) );
        return true;
    },
...
if (serverSide) {
    module.exports = {
        RRule: RRule,
        setDateClass: function (DateClass) {
            dateutil.Date = DateClass;
          //  dateutil.ORDINAL_BASE = new DateClass(dateutil.ORDINAL_BASE);
        }
        // rruleset: rruleset
    }
}
